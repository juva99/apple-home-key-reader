...
class RemoteUnlockService:
    ...
    async def _subscribe_and_listen(self):
        ...
        self._client = AsyncClient(
            ...
            AsyncClientOptions(
                realtime=RealtimeClientOptions(
                    ...
                    max_retries=float("inf"),  # Reasonable retry count
        ...
        log.info(
            f"✅ Subscribed to lock commands for {self.lock_id} on channel {channel_name}"
        )
        ...
        try:
            while self._running:
                ...
                if hasattr(self._channel, "state") and self._channel.state in [
                    "closed",
                    "errored",
                ]:
                    log.warning(
                        f"Channel state is {self._channel.state}, triggering reconnection..."
                    )
    ...
    def _on_subscribe_callback(
        self, status: RealtimeSubscribeStates, err: Optional[Exception]
    ):
