...
- `homekey`:
  ...
  - `lock_timeout`: number of seconds the lock stays unlocked after being opened via NFC authentication or manual unlock in the Home app before automatically locking again. Default is 10 seconds.
- `gpio`: GPIO configuration for Raspberry Pi (optional):
  - `pin`: BCM GPIO pin number to control (e.g., 18). Set to `null` or omit section to disable GPIO;
  - `duration`: how long to keep the GPIO pin HIGH in seconds (default: 2.0).
